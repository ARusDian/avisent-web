...
export function OperatorTurretControl() {
	...
	const handlePost = (task) => {
		...
		axios
			.post(`${turret.turretUrl}/task`, { task })
			.then((res) => {
				console.log(res);
			})
			.catch((err) => {
						...
						<div className="my-5 mx-20 grid grid-cols-3 gap-5">
							<div className="grid grid-cols-subgrid col-span-3">
								<button
									...
									onClick={() => handleClick("up2")}
									onMouseDown={() => onMouseDown("up2")}
									onMouseUp={onMouseUp}
								>
									<SlArrowUp className="m-5" />
								</button>
							</div>
							<button
								className="col-start-1 bg-blue-500 hover:bg-blue-700 text-white font-bold rounded flex flex-wrap justify-center"
								onClick={() => handleClick("left2")}
								onMouseDown={() => onMouseDown("left2")}
								onMouseUp={onMouseUp}
							>
								<SlArrowLeft className="m-5" />
							</button>
							<button
								className="col-start-3 bg-blue-500 hover:bg-blue-700 text-white font-bold rounded flex flex-wrap justify-center"
								onClick={() => handleClick("right2")}
								onMouseDown={() => onMouseDown("right2")}
								onMouseUp={onMouseUp}
							>
								<SlArrowRight className="m-5" />
							</button>
							<div className="grid grid-cols-subgrid col-span-3">
								<button
									className="col-start-2 bg-blue-500 hover:bg-blue-700 text-white font-bold rounded flex flex-wrap justify-center"
									onClick={() => handleClick("down2")}
									onMouseDown={() => onMouseDown("down2")}
									onMouseUp={onMouseUp}
								>
									<SlArrowDown className="m-5" />
								</button>
							</div>
						</div>
					) : (
